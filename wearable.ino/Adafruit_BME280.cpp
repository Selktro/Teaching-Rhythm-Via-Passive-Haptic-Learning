...
bool Adafruit_BME280::begin(uint8_t a) {
  ...
  if (_cs == -1) {
    ...
    if (_sck == -1) {
      // hardware SPI
      //Serial.println("init spi_master");
      SPI_Master.begin(A3, D2, D0);
      //SPI_Master.setSPIMode(SPI_MODE0);
      //SPI_Master.setFrequency(500000);
  ...
  //Serial.println(read8(BME280_REGISTER_CHIPID));
  //Serial.println(read8(BME280_REGISTER_CHIPID));
  //uint8_t res = read8(BME280_REGISTER_CHIPID);
  //Serial.println(res);
...
void Adafruit_BME280::beginTransaction() {
  SPI_Master.begin(A3, D2, D0);
  SPI_Master.setSPIMode(SPI_MODE0);
  SPI_Master.setFrequency(500000);
}
...
uint8_t Adafruit_BME280::spixfer(uint8_t x) {
  if (_sck == -1) {
    //uint8_t res = SPI_Master.transfer(x);
    //Serial.println(SPI_Master.transfer(x));
    return SPI_Master.transfer(x);
  }
...
void Adafruit_BME280::write8(byte reg, byte value)
{
  if (_cs == -1) {
    ...
    digitalWrite(_cs, HIGH);
    //if (_sck == -1)
      //SPI_Master.endTransfer();              // release the SPI bus
  }
}
